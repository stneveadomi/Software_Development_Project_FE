...
%[SCREEN_DIMENSIONS(3)/2-WIDTH/2 SCREEN_DIMENSIONS(4)/2-HEIGHT/2 WIDTH HEIGHT];
...
title('TRON','Color','white','FontName','Terminal','FontSize',20);
...
function bool=collision()
        ...
        for i=2:length(player1_body)
            if(isequal(player1_body(i,:),player1_body(1,:))||isequal(player2_body(i,:),player2_body(1,:))||isequal(player1_body(i,:),player2_body(1,:))||isequal(player2_body(i,:),player1_body(1,:)))
                bool=1;
                return;
            end
...
function setScore()
    xlabel(['PLAYER 1 SCORE: ',num2str(PLAYER1_SCORE),' PLAYER 2 SCORE: ',num2str(PLAYER2_SCORE)],'FontSize',20,'Color','w','FontName','Cambria');
end
